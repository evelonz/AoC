using AdventOfCode2020.Utility;
using FluentAssertions;
using System.Collections.Generic;
using System.Linq;
using Xunit;

namespace AdventOfCode2020.Day4
{
    internal static class Day4
    {
        internal static (int partOne, int partTwo) Solve1(IInputResolver input)
        {
            var data = input.AsEnumerable();

            var expected = new List<string>
            { "byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"
            }.OrderBy(o => o);

            var expectedWithCid = new List<string>
            { "byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid", "cid"
            }.OrderBy(o => o);

            int ans1 = 0;
            int ans2 = 0;
            var enumerator = data.GetEnumerator();
            while (enumerator.MoveNext())
            {
                var val = enumerator.Current;
                var fields = new List<string>(8);
                var fieldsComplete = new List<string>(8);
                var hasMore = true;
                while (val.Length != 0 && hasMore)
                {
                    var words = val.Split();
                    fieldsComplete.AddRange(words);
                    var field = words.Select(s => s[0..3]);
                    fields.AddRange(field);
                    hasMore = enumerator.MoveNext();
                    if(hasMore)
                        val = enumerator.Current;
                }
                fields = fields.OrderBy(o => o).ToList();
                if (fields.SequenceEqual(expectedWithCid) || fields.Where(x => x != "cid").SequenceEqual(expected))
                {
                    ans1++;
                    if (ValidateFields(fieldsComplete))
                        ans2++;
                }

            }

            return (ans1, ans2);
        }

        private static bool ValidateFields(List<string> inp)
        {
            string[] colours = { "amb", "blu", "brn", "gry", "grn", "hzl", "oth" };
            foreach (var item in inp)
            {
                var field = item[0..3];
                var data = item[4..];

                switch (field)
                {
                    case "byr":
                        if(!int.TryParse(data, out var byr))
                            return false;
                        if (byr < 1920 || byr > 2002)
                            return false;
                        break;
                    case "iyr":
                        if (!int.TryParse(data, out var iyr))
                            return false;
                        if (iyr < 2010 || iyr > 2020)
                            return false;
                        break;
                    case "eyr":
                        if (!int.TryParse(data, out var eyr))
                            return false;
                        if (eyr < 2020 || eyr > 2030)
                            return false;
                        break;
                    case "hgt":
                        var r = data[0..^2];
                        if (!int.TryParse(r, out var hgt))
                            return false;
                        if (data.EndsWith("in"))
                        {
                            if (hgt < 59 || hgt > 76)
                                return false;
                        }
                        else if (data.EndsWith("cm"))
                        {
                            if (hgt < 150 || hgt > 193)
                                return false;
                        }
                        else
                            return false;
                        break;
                    case "hcl":
                        if (!data.StartsWith('#'))
                            return false;
                        foreach (var ch in data[1..])
                        {
                            if (!((ch >= 'a' && ch <= 'f') || (ch >= '0' && ch <= '9')))
                                return false;
                        }
                        break;
                    case "ecl":
                        if (!colours.Contains(data))
                            return false;
                        break;
                    case "pid":
                        if (data.Length != 9 || !int.TryParse(data, out var pid))
                            return false;
                        break;
                    case "cid":
                        break;
                }
            }
            return true;
        }

    }

    public class Test2020Day4
    {
        [Theory]
        [MemberData(nameof(ExampleData))]
        public void SolveProblemExamples(string[] example, int expectedFirst, int expectedSecond)
        {
            var (partOne, partTwo) = Day4
                .Solve1(new MockInputResolver(example));

            partOne.Should().Be(expectedFirst);
            partTwo.Should().Be(expectedSecond);
        }

        [Fact]
        public void SolveProblemInput()
        {
            var (partOne, partTwo) = Day4
                .Solve1(new FileInputResolver(4));

            partOne.Should().Be(222);
            partTwo.Should().Be(140);
        }

        public readonly static List<object[]> ExampleData = new List<object[]>
        {
            new object[] {
                new [] {
                    "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd",
                    "byr:1937 iyr:2017 cid:147 hgt:183cm",
                    "",
                    "iyr:2013 ecl:amb cid:350 eyr:2023 pid:028048884",
                    "hcl:#cfa07d byr:1929",
                    "",
                    "hcl:#ae17e1 iyr:2013",
                    "eyr:2024",
                    "ecl:brn pid:760753108 byr:1931",
                    "hgt:179cm",
                    "",
                    "hcl:#cfa07d eyr:2025 pid:166559648",
                    "iyr:2011 ecl:brn hgt:59in",}
                , 2, 2
            },
            new object[] {
                new [] {
                    "eyr:1972 cid:100",
                    "hcl:#18171d ecl:amb hgt:170 pid:186cm iyr:2018 byr:1926",
                    "",
                    "iyr:2019",
                    "hcl:#602927 eyr:1967 hgt:170cm",
                    "ecl:grn pid:012533040 byr:1946",
                    "",
                    "hcl:dab227 iyr:2012",
                    "ecl:brn hgt:182cm pid:021572410 eyr:2020 byr:1992 cid:277",
                    "",
                    "hgt:59cm ecl:zzz",
                    "eyr:2038 hcl:74454a iyr:2023",
                    "pid:3556412378 byr:2007",
                }, 4, 0
            },
            new object[] {
                new [] {
                    "pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980",
                    "hcl:#623a2f",
                    "",
                    "eyr:2029 ecl:blu cid:129 byr:1989",
                    "iyr:2014 pid:896056539 hcl:#a97842 hgt:165cm",
                    "",
                    "hcl:#888785",
                    "hgt:164cm byr:2001 iyr:2015 cid:88",
                    "pid:545766238 ecl:hzl",
                    "eyr:2022",
                    "",
                    "iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719",
                }, 4, 4
            },
        };
    }
}
